...
public class SieveOfEratosthenes {
    ...
    public static boolean[] sieve(int n) { 
        ...
        //here we build the sieve's tables itself. A list of all numbers from 2 to n will be initially prime numbers.
        boolean[] isPrime = new boolean[n + 1]; // create a boolean array to store the prime status of each number
        for (int i = 2; i <= n; i++) {  // Initialize all numbers as prime because, initially, we assume that all numbers are prime!
            isPrime[i] = true;
        }
        
        // Mark multiples of each prime as non-prime
        for (int i = 2; i * i <= n; i++) { //for loop until square root of n.(I*I <= N) = math.sqrt(n), but calling the square root is more expensive.
            if (isPrime[i]) {//if i is prime, then all its multiples are not prime.
                for (int j = i * i; j <= n; j += i) { //mark all multiples of i as non-prime.
                    isPrime[j] = false; //marking as non-prime startng from I*I because all the numbers before are already marked
                }
            }
        }
